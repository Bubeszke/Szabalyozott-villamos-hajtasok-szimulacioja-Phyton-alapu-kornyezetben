...
class GenericController:
    def __init__(self, environment, tau):
        ...
        self.t32 = environment.physical_system.electrical_motor.t_32
...
class FOC_Controller(GenericController):
    ...
    def backward_transformation(self, quantities, eps):
        reversed_quantities = quantities[::-1]
        result_of_q = self.q(reversed_quantities, eps)
        final_result = self.t32(result_of_q)
        return final_result
...
class MPC_Controller(FOC_Controller):
    ...
    def control(self, state, reference):
        ...
        for i in range(self.prediction_horizon): # Load the data into the future dimensions
            refrence_D.append(reference["D"])
            refrence_Q.append(reference["Q"])
        ...
        u_a, u_b, u_c = self.backward_transformation((u_q.NEWVAL, u_d.NEWVAL), epsilon_prev)
